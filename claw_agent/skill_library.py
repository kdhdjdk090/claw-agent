...
SKILL_REGISTRY: tuple[SkillEntry, ...] = (
    ...
    # ═══════════════════════════════════════════════════════════════
    # EXPANSION WAVE 3 — Push to 660+ clone-level coverage
    # ═══════════════════════════════════════════════════════════════

    # ── document-processing +15 ──────────────────────────────────
    SkillEntry("pdf-encryption", CAT_DOCUMENT,
        "Encrypt, decrypt, and password-protect PDF documents",
        ("pdf encryption", "pdf password", "encrypt pdf", "decrypt pdf")),
    SkillEntry("pdf-watermark", CAT_DOCUMENT,
        "Add text or image watermarks to PDF pages",
        ("pdf watermark", "watermark pdf", "stamp pdf")),
    SkillEntry("document-diffing", CAT_DOCUMENT,
        "Compare two document versions and highlight changes",
        ("document diff", "compare documents", "doc comparison", "track changes")),
    SkillEntry("ebook-creation", CAT_DOCUMENT,
        "Create EPUB and MOBI ebooks from markdown or HTML sources",
        ("ebook", "epub", "mobi", "create ebook", "ebook creation")),
    SkillEntry("invoice-generation", CAT_DOCUMENT,
        "Generate professional invoices from structured data",
        ("invoice", "generate invoice", "invoice template", "billing document")),
    SkillEntry("receipt-parsing", CAT_DOCUMENT,
        "Extract structured data from receipt images using OCR",
        ("receipt parsing", "parse receipt", "receipt ocr", "scan receipt")),
    SkillEntry("contract-analysis", CAT_DOCUMENT,
        "Analyze legal contracts and extract key terms and obligations",
        ("contract analysis", "analyze contract", "legal document", "contract review")),
    SkillEntry("resume-parsing", CAT_DOCUMENT,
        "Parse resumes and CVs into structured candidate data",
        ("resume parsing", "parse resume", "cv parser", "resume extractor")),
    SkillEntry("document-signing", CAT_DOCUMENT,
        "Implement digital signature workflows for documents",
        ("document signing", "digital signature", "e-signature", "sign document")),
    SkillEntry("mail-merge", CAT_DOCUMENT,
        "Generate personalized documents from templates and data sources",
        ("mail merge", "document template", "personalized document", "merge fields")),
    SkillEntry("pdf-accessibility", CAT_DOCUMENT,
        "Make PDFs accessible with proper tags, reading order, and alt text",
        ("pdf accessibility", "accessible pdf", "pdf tags", "pdf 508")),
    SkillEntry("document-versioning", CAT_DOCUMENT,
        "Track document versions with metadata and change history",
        ("document version", "version control document", "document history")),
    SkillEntry("batch-document-processing", CAT_DOCUMENT,
        "Process large batches of documents with parallel pipelines",
        ("batch processing", "bulk documents", "document pipeline", "batch pdf")),
    SkillEntry("document-classification", CAT_DOCUMENT,
        "Classify documents by type using ML or rule-based approaches",
        ("document classification", "classify document", "document type", "document categorization")),

    # ── blockchain-web3 +15 ──────────────────────────────────────
    SkillEntry("zk-proofs", CAT_BLOCKCHAIN,
        "Implement zero-knowledge proof circuits and verifiers",
        ("zero knowledge", "zk proof", "zk-snark", "zk-stark", "zero-knowledge proof")),
    SkillEntry("token-vesting", CAT_BLOCKCHAIN,
        "Create token vesting schedules with cliff and linear unlock",
        ("token vesting", "vesting schedule", "cliff vesting", "unlock schedule")),
    SkillEntry("yield-farming", CAT_BLOCKCHAIN,
        "Build yield farming and liquidity mining protocols",
        ("yield farming", "liquidity mining", "farming protocol", "yield aggregator")),
    SkillEntry("web3-indexing", CAT_BLOCKCHAIN,
        "Index blockchain events with subgraphs or custom indexers",
        ("subgraph", "blockchain indexing", "event indexing", "the graph")),
    SkillEntry("stablecoin-design", CAT_BLOCKCHAIN,
        "Design algorithmic or collateralized stablecoin mechanisms",
        ("stablecoin", "algorithmic stablecoin", "collateralized stablecoin")),
    SkillEntry("mev-protection", CAT_BLOCKCHAIN,
        "Protect transactions from MEV extraction and sandwich attacks",
        ("mev protection", "sandwich attack", "frontrunning", "mev")),
    SkillEntry("on-chain-analytics", CAT_BLOCKCHAIN,
        "Analyze on-chain data for wallet profiling and trend detection",
        ("on-chain analytics", "blockchain analytics", "wallet analysis", "chain analysis")),
    SkillEntry("smart-contract-upgrade", CAT_BLOCKCHAIN,
        "Implement upgradeable smart contract patterns (proxy, diamond)",
        ("upgradeable contract", "proxy pattern", "diamond pattern", "contract upgrade")),
    SkillEntry("token-launchpad", CAT_BLOCKCHAIN,
        "Create fair-launch or IDO platforms for new token offerings",
        ("launchpad", "ido", "token launch", "fair launch", "initial offering")),

    # ── data-analytics +12 ───────────────────────────────────────
    SkillEntry("data-lakehouse", CAT_DATA,
        "Design data lakehouse architectures combining warehouse and lake patterns",
        ("data lakehouse", "lakehouse", "delta lake", "iceberg")),
    SkillEntry("feature-store", CAT_DATA,
        "Build feature stores for ML feature reuse and consistency",
        ("feature store", "ml features", "feature engineering store", "feast")),
    SkillEntry("data-catalog", CAT_DATA,
        "Implement data catalogs for discovery, lineage, and governance",
        ("data catalog", "data discovery", "data lineage", "data governance catalog")),
    SkillEntry("real-time-analytics", CAT_DATA,
        "Build real-time analytics pipelines with streaming engines",
        ("real-time analytics", "streaming analytics", "real time data", "live analytics")),
    SkillEntry("data-mesh", CAT_DATA,
        "Implement data mesh patterns with domain-oriented ownership",
        ("data mesh", "domain data", "data product", "data mesh architecture")),
    SkillEntry("data-contracts", CAT_DATA,
        "Define and enforce data contracts between producers and consumers",
        ("data contract", "schema contract", "data interface", "data agreement")),
    SkillEntry("reverse-etl", CAT_DATA,
        "Sync transformed warehouse data back to operational tools",
        ("reverse etl", "warehouse sync", "data activation", "operational analytics")),
    SkillEntry("data-observability", CAT_DATA,
        "Monitor data quality, freshness, and volume with observability tools",
        ("data observability", "data quality monitoring", "data freshness", "monte carlo")),
    SkillEntry("geospatial-analytics", CAT_DATA,
        "Analyze and visualize geospatial data with PostGIS, H3, or Mapbox",
        ("geospatial", "gis", "postgis", "spatial analytics", "geospatial data")),
    SkillEntry("graph-analytics", CAT_DATA,
        "Run graph algorithms for network analysis and relationship discovery",
        ("graph analytics", "network analysis", "graph algorithm", "neo4j analytics")),
    SkillEntry("time-series-forecasting", CAT_DATA,
        "Build time-series forecasting models (ARIMA, Prophet, neural)",
        ("time series", "forecasting", "prophet", "arima", "time series forecasting")),
    SkillEntry("data-anonymization", CAT_DATA,
        "Anonymize and pseudonymize datasets for privacy compliance",
        ("data anonymization", "pseudonymization", "data masking", "pii removal")),

    # ── design-ui +12 ────────────────────────────────────────────
    SkillEntry("dark-mode-design", CAT_DESIGN_UI,
        "Design adaptive dark mode color systems and theming",
        ("dark mode", "dark theme", "adaptive color", "dark mode design")),
    SkillEntry("typography-system", CAT_DESIGN_UI,
        "Build comprehensive typography scales and font pairing systems",
        ("typography system", "type scale", "font pairing", "typography")),
    SkillEntry("illustration-system", CAT_DESIGN_UI,
        "Create consistent illustration styles and asset libraries",
        ("illustration", "illustration system", "icon illustration", "brand illustration")),
    SkillEntry("design-handoff", CAT_DESIGN_UI,
        "Prepare pixel-perfect design handoff packages for developers",
        ("design handoff", "developer handoff", "spec handoff", "design to dev")),
    SkillEntry("ux-writing", CAT_DESIGN_UI,
        "Write clear microcopy, error messages, and interface text",
        ("ux writing", "microcopy", "interface text", "ui copy", "error message")),
    SkillEntry("design-audit", CAT_DESIGN_UI,
        "Audit existing UI for inconsistencies, accessibility, and usability issues",
        ("design audit", "ui audit", "usability audit", "ux audit")),
    SkillEntry("spacing-grid-system", CAT_DESIGN_UI,
        "Design 4/8px spacing scales and responsive grid systems",
        ("spacing system", "grid system", "spacing scale", "layout grid")),
    SkillEntry("color-system-design", CAT_DESIGN_UI,
        "Build systematic color palettes with semantic tokens and contrast ratios",
        ("color system", "color palette", "semantic color", "color tokens")),
    SkillEntry("user-flow-mapping", CAT_DESIGN_UI,
        "Map complete user journeys and task flows with decision points",
        ("user flow", "task flow", "user journey map", "flow mapping")),
    SkillEntry("design-sprint", CAT_DESIGN_UI,
        "Facilitate Google Ventures-style design sprints for rapid prototyping",
        ("design sprint", "gv sprint", "rapid prototyping", "sprint prototype")),

    # ── mobile +12 ───────────────────────────────────────────────
    SkillEntry("mobile-deep-linking", CAT_MOBILE,
        "Implement universal links, deep links, and deferred deep linking",
        ("deep link", "universal link", "app link", "deferred deep link", "deep linking")),
    SkillEntry("mobile-push-notification", CAT_MOBILE,
        "Build push notification systems with segmentation and scheduling",
        ("push notification", "mobile notification", "fcm", "apns", "push")),
    SkillEntry("mobile-offline-first", CAT_MOBILE,
        "Design offline-first mobile architectures with sync conflict resolution",
        ("offline first", "mobile offline", "sync conflict", "offline mobile")),
    SkillEntry("mobile-app-security", CAT_MOBILE,
        "Harden mobile apps against reverse engineering, tampering, and data leaks",
        ("mobile security", "app hardening", "certificate pinning", "mobile encryption")),
    SkillEntry("mobile-payment-integration", CAT_MOBILE,
        "Integrate in-app purchases, subscriptions, and payment SDKs",
        ("in-app purchase", "mobile payment", "iap", "storekit", "google billing")),
    SkillEntry("mobile-crash-reporting", CAT_MOBILE,
        "Set up crash reporting and monitoring (Crashlytics, Sentry, Bugsnag)",
        ("crash reporting", "crashlytics", "mobile crash", "crash monitoring")),
    SkillEntry("mobile-biometrics", CAT_MOBILE,
        "Implement biometric authentication (Face ID, fingerprint, iris) in mobile apps",
        ("biometrics", "face id", "fingerprint auth", "biometric authentication")),
    SkillEntry("mobile-camera-ar", CAT_MOBILE,
        "Build camera features and augmented reality with ARKit/ARCore",
        ("arkit", "arcore", "augmented reality", "mobile camera", "ar mobile")),
    SkillEntry("mobile-localization", CAT_MOBILE,
        "Localize mobile apps with RTL support, date/number formats, and translations",
        ("mobile localization", "app localization", "rtl support", "mobile i18n")),
    SkillEntry("mobile-widget", CAT_MOBILE,
        "Build home screen widgets (WidgetKit, Glance, App Widgets)",
        ("widget", "widgetkit", "home screen widget", "app widget", "glance widget")),

    # ── game-dev +12 ─────────────────────────────────────────────
    SkillEntry("game-audio", CAT_GAME_DEV,
        "Game audio systems — adaptive music, spatial audio, sound design pipelines",
        ("game audio", "adaptive music", "spatial audio", "game sound")),
    SkillEntry("game-localization", CAT_GAME_DEV,
        "Localize games across languages including string tables, font fallbacks, and VO",
        ("game localization", "game l10n", "game translation", "localized game")),
    SkillEntry("game-save-system", CAT_GAME_DEV,
        "Design robust save/load systems with versioning and cloud sync",
        ("save system", "game save", "save load", "cloud save", "save versioning")),
    SkillEntry("game-input-system", CAT_GAME_DEV,
        "Build flexible input systems supporting gamepad, keyboard, touch, and rebinding",
        ("input system", "gamepad", "input rebinding", "controller support")),
    SkillEntry("voxel-engine", CAT_GAME_DEV,
        "Build voxel-based worlds with chunk management and meshing algorithms",
        ("voxel", "voxel engine", "chunk management", "marching cubes")),
    SkillEntry("game-replay-system", CAT_GAME_DEV,
        "Record and replay gameplay with deterministic simulation or keyframe capture",
        ("replay system", "game replay", "demo recording", "instant replay")),
    SkillEntry("game-achievement-system", CAT_GAME_DEV,
        "Design achievement and trophy systems with platform integration",
        ("achievement system", "trophy", "game achievements", "steam achievements")),
    SkillEntry("game-dialogue-system", CAT_GAME_DEV,
        "Build branching dialogue trees with conditions, variables, and ink/yarn integration",
        ("dialogue system", "dialogue tree", "conversation system", "ink", "yarn spinner")),
    SkillEntry("game-tutorial-design", CAT_GAME_DEV,
        "Design progressive onboarding tutorials that teach without boring",
        ("game tutorial", "player onboarding", "tutorial design", "teaching mechanics")),
    SkillEntry("game-anti-cheat", CAT_GAME_DEV,
        "Implement server-authoritative anti-cheat and exploit detection",
        ("anti-cheat", "game security", "cheat detection", "exploit prevention")),

    # ── writing-content +12 ──────────────────────────────────────
    SkillEntry("ghostwriting", CAT_WRITING,
        "Ghostwrite in a target author's voice using style analysis and voice matching",
        ("ghostwriting", "ghostwrite", "write in voice", "voice matching")),
    SkillEntry("speech-writing", CAT_WRITING,
        "Write compelling speeches with rhetorical structure and delivery notes",
        ("speech writing", "keynote speech", "public speaking", "speech draft")),
    SkillEntry("grant-writing", CAT_WRITING,
        "Write grant proposals with budget justification and research methodology",
        ("grant writing", "grant proposal", "research grant", "funding application")),
    SkillEntry("video-script-writing", CAT_WRITING,
        "Write video and YouTube scripts with hooks, pacing, and B-roll notes",
        ("video script", "youtube script", "script writing", "video writing")),
    SkillEntry("brand-naming", CAT_WRITING,
        "Generate and evaluate brand names, product names, and feature names",
        ("brand naming", "product name", "naming", "name generation")),
    SkillEntry("tagline-creation", CAT_WRITING,
        "Craft memorable taglines and slogans through iterative refinement",
        ("tagline", "slogan", "catchphrase", "brand tagline")),
    SkillEntry("technical-editing", CAT_WRITING,
        "Edit technical documentation for accuracy, clarity, and consistency",
        ("technical editing", "tech editing", "documentation editing", "style editing")),
    SkillEntry("content-localization", CAT_WRITING,
        "Adapt content for different markets with cultural and linguistic sensitivity",
        ("content localization", "localize content", "cultural adaptation", "transcreation")),
    SkillEntry("changelog-writing", CAT_WRITING,
        "Write user-facing changelogs from commit history and release notes",
        ("changelog", "release notes writing", "changelog writing", "version notes")),

    # ── frontend +10 ─────────────────────────────────────────────
    SkillEntry("web-animation", CAT_FRONTEND,
        "Build performant web animations with GSAP, Framer Motion, or CSS",
        ("web animation", "gsap", "framer motion", "css animation")),
    SkillEntry("web-audio", CAT_FRONTEND,
        "Build Web Audio API applications — synthesizers, visualizers, audio processing",
        ("web audio", "audio api", "web synthesizer", "audio visualizer")),
    SkillEntry("webgl-3d", CAT_FRONTEND,
        "3D web experiences with Three.js, WebGL, and WebGPU",
        ("webgl", "three.js", "threejs", "webgpu", "3d web")),
    SkillEntry("pwa-advanced", CAT_FRONTEND,
        "Advanced PWA features — background sync, push, share target, periodic sync",
        ("pwa advanced", "background sync", "share target", "periodic sync")),
    SkillEntry("web-components-lit", CAT_FRONTEND,
        "Build framework-agnostic web components with Lit or vanilla custom elements",
        ("web components", "lit element", "custom element", "shadow dom")),
    SkillEntry("frontend-monorepo", CAT_FRONTEND,
        "Manage frontend monorepos with Nx, Turborepo, or Lerna",
        ("frontend monorepo", "nx workspace", "turborepo", "lerna")),
    SkillEntry("browser-extension", CAT_FRONTEND,
        "Build cross-browser extensions (Chrome, Firefox, Safari) with manifest v3",
        ("browser extension", "chrome extension", "manifest v3", "firefox addon")),
    SkillEntry("rich-text-editor", CAT_FRONTEND,
        "Build rich text editors with ProseMirror, TipTap, Slate, or Lexical",
        ("rich text editor", "prosemirror", "tiptap", "slate editor", "lexical")),
    SkillEntry("image-optimization", CAT_FRONTEND,
        "Optimize images with modern formats (AVIF, WebP), lazy loading, and CDN delivery",
        ("image optimization", "avif", "webp", "lazy loading images", "image cdn")),

    # ── devops-infra +10 ─────────────────────────────────────────
    SkillEntry("gitops", CAT_DEVOPS,
        "Implement GitOps workflows with ArgoCD or Flux for declarative deployments",
        ("gitops", "argocd", "flux cd", "declarative deployment", "gitops workflow")),
    SkillEntry("service-mesh", CAT_DEVOPS,
        "Deploy service meshes (Istio, Linkerd) for traffic management and observability",
        ("service mesh", "istio", "linkerd", "envoy proxy")),
    SkillEntry("infrastructure-testing", CAT_DEVOPS,
        "Test infrastructure with Terratest, InSpec, or Checkov",
        ("infrastructure testing", "terratest", "inspec", "infra testing")),
    SkillEntry("container-orchestration", CAT_DEVOPS,
        "Advanced Kubernetes patterns — operators, CRDs, admission webhooks",
        ("kubernetes operator", "crd", "admission webhook", "k8s operator")),
    SkillEntry("cost-optimization-cloud", CAT_DEVOPS,
        "Optimize cloud costs with rightsizing, reserved instances, and spot",
        ("cloud cost", "cost optimization", "rightsizing", "reserved instance", "spot instance")),
    SkillEntry("database-reliability", CAT_DEVOPS,
        "High availability database patterns — replication, failover, backup strategies",
        ("database ha", "replication", "failover", "database backup", "db reliability")),
    SkillEntry("dns-management", CAT_DEVOPS,
        "Manage DNS records, DNSSEC, and multi-region DNS routing",
        ("dns management", "dnssec", "route53", "dns routing")),
    SkillEntry("edge-computing", CAT_DEVOPS,
        "Deploy at the edge with Cloudflare Workers, Deno Deploy, or Lambda@Edge",
        ("edge computing", "cloudflare workers", "deno deploy", "lambda edge")),

    # ── security +10 ─────────────────────────────────────────────
    SkillEntry("oauth-oidc-implementation", CAT_SECURITY,
        "Implement OAuth 2.0 and OpenID Connect flows correctly",
        ("oauth", "oidc", "openid connect", "oauth2 flow", "authorization code")),
    SkillEntry("security-headers", CAT_SECURITY,
        "Configure HTTP security headers (CSP, HSTS, X-Frame, Permissions-Policy)",
        ("security headers", "csp", "hsts", "content security policy")),
    SkillEntry("waf-configuration", CAT_SECURITY,
        "Configure web application firewalls with custom rules and rate limiting",
        ("waf", "web application firewall", "waf rules", "rate limiting waf")),
    SkillEntry("cryptography-implementation", CAT_SECURITY,
        "Implement cryptographic operations correctly — hashing, signing, encryption",
        ("cryptography", "encryption implementation", "digital signing", "hashing")),
    SkillEntry("security-compliance-frameworks", CAT_SECURITY,
        "Map controls to SOC 2, ISO 27001, PCI DSS, and HIPAA frameworks",
        ("soc 2", "iso 27001", "pci dss", "hipaa compliance", "compliance framework")),
    SkillEntry("api-security", CAT_SECURITY,
        "Secure APIs with rate limiting, input validation, auth, and abuse prevention",
        ("api security", "api rate limit", "api auth", "api abuse")),
    SkillEntry("security-incident-response", CAT_SECURITY,
        "Build incident response playbooks with triage, containment, and post-mortem",
        ("incident response", "security incident", "breach response", "security playbook")),

    # ── testing-qa +10 ───────────────────────────────────────────
    SkillEntry("chaos-testing", CAT_TESTING,
        "Test system resilience with fault injection and chaos experiments",
        ("chaos testing", "fault injection", "resilience testing", "chaos experiment")),
    SkillEntry("api-testing-automation", CAT_TESTING,
        "Automate API testing with Postman/Newman, REST Assured, or k6",
        ("api testing", "postman testing", "rest assured", "api automation")),
    SkillEntry("flaky-test-management", CAT_TESTING,
        "Detect, quarantine, and fix flaky tests in CI pipelines",
        ("flaky test", "flaky tests", "test flakiness", "quarantine test")),
    SkillEntry("smoke-testing-strategy", CAT_TESTING,
        "Design smoke and sanity test suites for deployment verification",
        ("smoke test", "sanity test", "deployment verification", "smoke suite")),
    SkillEntry("test-coverage-analysis", CAT_TESTING,
        "Analyze and improve test coverage targeting meaningful metrics",
        ("test coverage", "code coverage", "coverage analysis", "coverage gaps")),

    # ── ai-ml +8 ─────────────────────────────────────────────────
    SkillEntry("llm-fine-tuning", CAT_AI_ML,
        "Fine-tune language models with LoRA, QLoRA, and full fine-tuning",
        ("fine-tuning", "fine tune", "lora", "qlora", "llm fine-tuning")),
    SkillEntry("agent-framework", CAT_AI_ML,
        "Build autonomous AI agents with tool use, planning, and memory",
        ("ai agent", "agent framework", "autonomous agent", "tool use agent")),
    SkillEntry("embedding-search", CAT_AI_ML,
        "Build semantic search with embeddings and vector similarity",
        ("embedding search", "semantic search", "vector similarity", "embedding")),
    SkillEntry("llm-guardrails", CAT_AI_ML,
        "Implement guardrails for LLM safety — content filtering, output validation",
        ("llm guardrails", "ai safety", "content filtering", "output validation")),
    SkillEntry("ml-model-serving", CAT_AI_ML,
        "Serve ML models in production with TorchServe, Triton, or BentoML",
        ("model serving", "torchserve", "triton", "bentoml", "ml serving")),
    SkillEntry("multimodal-ai", CAT_AI_ML,
        "Build multimodal AI applications combining text, image, and audio",
        ("multimodal", "multimodal ai", "vision language", "text image")),

    # ── productivity +8 ──────────────────────────────────────────
    SkillEntry("meeting-notes-automation", CAT_PRODUCTIVITY,
        "Automate meeting transcription, summarization, and action item extraction",
        ("meeting notes", "meeting summary", "action items", "meeting transcription")),
    SkillEntry("personal-knowledge-management", CAT_PRODUCTIVITY,
        "Build second-brain systems with Obsidian, Notion, or Logseq",
        ("second brain", "knowledge management", "obsidian vault", "zettelkasten")),
    SkillEntry("time-tracking-automation", CAT_PRODUCTIVITY,
        "Automate time tracking and reporting for teams and freelancers",
        ("time tracking", "time management", "pomodoro", "time report")),
    SkillEntry("email-automation", CAT_PRODUCTIVITY,
        "Automate email workflows — filters, templates, scheduling, follow-ups",
        ("email automation", "email workflow", "email filter", "automated email")),
    SkillEntry("task-decomposition", CAT_PRODUCTIVITY,
        "Break complex tasks into actionable subtasks with dependency tracking",
        ("task breakdown", "task decomposition", "subtask", "work breakdown")),
    SkillEntry("weekly-review-system", CAT_PRODUCTIVITY,
        "GTD-style weekly review system with reflection prompts and planning",
        ("weekly review", "gtd review", "reflection", "weekly planning")),
    SkillEntry("decision-framework", CAT_PRODUCTIVITY,
        "Apply structured decision frameworks (RAPID, DACI, decision matrix)",
        ("decision framework", "decision matrix", "rapid", "daci")),
    SkillEntry("automation-scripting", CAT_PRODUCTIVITY,
        "Write automation scripts for repetitive tasks across platforms",
        ("automation script", "automate tasks", "scripting", "task automation")),

    # ── specialized-domains +8 ───────────────────────────────────
    SkillEntry("healthcare-hl7-fhir", CAT_SPECIALIZED,
        "Implement HL7 FHIR APIs for healthcare data interoperability",
        ("hl7", "fhir", "healthcare api", "medical data", "hl7 fhir")),
    SkillEntry("fintech-payments", CAT_SPECIALIZED,
        "Build payment processing systems with PCI compliance and settlement",
        ("payment processing", "pci compliance", "payment gateway", "settlement")),
    SkillEntry("legaltech-contracts", CAT_SPECIALIZED,
        "Build contract management and CLM systems for legal teams",
        ("contract management", "clm", "legal tech", "contract lifecycle")),
    SkillEntry("edtech-lms", CAT_SPECIALIZED,
        "Build learning management systems with progress tracking and assessments",
        ("lms", "learning management", "edtech", "course platform")),
    SkillEntry("proptech-real-estate", CAT_SPECIALIZED,
        "Build proptech solutions — listings, virtual tours, property management",
        ("proptech", "real estate tech", "property management", "virtual tour")),
    SkillEntry("agritech-iot", CAT_SPECIALIZED,
        "Build agricultural IoT systems for monitoring and precision farming",
        ("agritech", "precision farming", "agricultural iot", "farm monitoring")),
    SkillEntry("logistics-tms", CAT_SPECIALIZED,
        "Build transportation management systems with route optimization",
        ("tms", "transportation management", "route optimization", "logistics system")),
    SkillEntry("govtech-civic", CAT_SPECIALIZED,
        "Build civic technology for government services and citizen engagement",
        ("govtech", "civic tech", "government tech", "citizen portal")),

    # ── engineering-core +5 ──────────────────────────────────────
    SkillEntry("monorepo-management", CAT_ENGINEERING_CORE,
        "Manage monorepos with Nx, Turborepo, Bazel, or Pants",
        ("monorepo", "nx monorepo", "turborepo management", "bazel")),
    SkillEntry("code-generation", CAT_ENGINEERING_CORE,
        "Build code generators and scaffolding tools with templates",
        ("code generation", "code scaffold", "code template", "generator")),
    SkillEntry("technical-debt-management", CAT_ENGINEERING_CORE,
        "Identify, track, and systematically pay down technical debt",
        ("technical debt", "tech debt", "debt management", "refactoring plan")),
    SkillEntry("interview-system-design", CAT_ENGINEERING_CORE,
        "Design system design interview questions and evaluation rubrics",
        ("system design interview", "interview rubric", "hiring system design")),
    SkillEntry("open-source-maintenance", CAT_ENGINEERING_CORE,
        "Maintain open source projects — issues, PRs, releases, community",
        ("open source", "oss maintenance", "open source project", "community management")),

    # ── backend +5 ───────────────────────────────────────────────
    SkillEntry("event-sourcing", CAT_BACKEND,
        "Implement event sourcing with event stores and projection rebuilding",
        ("event sourcing", "event store", "cqrs event", "event replay")),
    SkillEntry("job-queue-processing", CAT_BACKEND,
        "Build reliable job queues with BullMQ, Celery, Sidekiq, or SQS",
        ("job queue", "bullmq", "celery", "sidekiq", "background jobs")),
    SkillEntry("file-upload-processing", CAT_BACKEND,
        "Handle large file uploads with chunking, resumption, and processing",
        ("file upload", "chunked upload", "multipart upload", "upload processing")),
    SkillEntry("webhook-system", CAT_BACKEND,
        "Build webhook delivery systems with retry, signing, and dead-letter queues",
        ("webhook system", "webhook delivery", "webhook retry", "webhook signing")),
    SkillEntry("multi-tenancy", CAT_BACKEND,
        "Implement multi-tenant architectures with data isolation strategies",
        ("multi-tenancy", "multi tenant", "tenant isolation", "saas multi-tenant")),

    # ── business-strategy +5 ─────────────────────────────────────
    SkillEntry("pitch-deck-creation", CAT_BUSINESS,
        "Create investor pitch decks with story arc and data visualization",
        ("pitch deck", "investor deck", "fundraising deck", "pitch presentation")),
    SkillEntry("board-reporting", CAT_BUSINESS,
        "Create board meeting materials with KPI dashboards and strategic narrative",
        ("board reporting", "board deck", "board meeting", "investor update")),

    # ── marketing-growth +5 ──────────────────────────────────────
    SkillEntry("viral-loop-design", CAT_MARKETING,
        "Design viral loop mechanics — referral, sharing, network effects",
        ("viral loop", "viral mechanic", "growth loop", "network effect")),
    SkillEntry("lifecycle-marketing", CAT_MARKETING,
        "Design full customer lifecycle email and notification strategies",
        ("lifecycle marketing", "lifecycle email", "customer lifecycle", "retention marketing")),
    SkillEntry("demand-generation", CAT_MARKETING,
        "Build demand gen engines — content syndication, webinars, ABM",
        ("demand generation", "demand gen", "abm", "account based marketing")),
    SkillEntry("marketing-attribution", CAT_MARKETING,
        "Implement multi-touch attribution models for marketing spend optimization",
        ("marketing attribution", "multi-touch attribution", "attribution model", "marketing roi")),
